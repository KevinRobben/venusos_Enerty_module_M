#!/bin/bash
#
# Copied from https://github.com/kwindrem/SetupHelper
#
...
script="/data/module_m/install.sh"
if [ -f "$script" ]; then
	logMessage "running module_m/install.sh"
    chmod +x "$script" # Ensure script is executable
    nohup "$script" > $logDir/install.log 2>&1 &
    logMessage "install.sh called, check $logDir/install.log for details"
else
    logMessage "install.sh not found at $script"
fi

# Play notification to let the user know the script is done
# notify "Module-M installed"

logMessage "completed"

# echo "### ${script} done - rebooting"
# # Reboot to start the script via rc.local
# reboot